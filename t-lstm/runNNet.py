...
def run(args=None):
    ...
    elif(opts.model=='TLSTM'):
        nn = TLSTM(opts.wvecDim,opts.middleDim,opts.outputDim,opts.numWords,opts.minibatch)
    ...
    for e in range(opts.epochs):
        ...
        if evaluate_accuracy_while_training:
            ...
            train_accuracies.append(test(opts.outFile,"train",opts.model,trees, e==opts.epochs-1))
            print "testing on dev set real quick"
            dev_accuracies.append(test(opts.outFile,"dev",opts.model,dev_trees, e==opts.epochs-1))
	...
	plt.plot(range(1, 1+len(train_accuracies)), train_accuracies)
	plt.hold('on')
	plt.plot(range(1, 1+len(dev_accuracies)), dev_accuracies)
...
def test(netFile,dataSet, model='RNN', trees=None, conf_flag=0):
    if trees==None:
